...
module.exports.mining = async () => {
  let run = async (timeout, func, maxErrors) => {
    let iteration = 0;
    let channel = await client.channels.resolve('833321015168860160');
    let execute = async () => {
  ...
  let getOnlineMembers = async () => {
    try {
      let guild = client.guilds.resolve('831878963839107112')
  ...
  //console.log(await run(1000, getOnlineMembers, 5))

  let startMining = async () => {
    let a = performance.now();
    let miners = await run(1300, getOnlineMembers, 10);
    ...
    let mineIteration = async () => {
      ...
      if (Date.now() < endDate) {
        ...
        a = performance.now();
        let onlineMembers = await run(1300, getOnlineMembers, 10);
        b = performance.now();
