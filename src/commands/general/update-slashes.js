...
module.exports.help = {
  name: `/${commandName}`,
  description: `_Позволяет разработчику обновить слэш команды на сервере._
...
async function commandExecution(interaction) {
  ...
  if (!server) {
    try {
      ...
      interaction.reply({ content: `Слэш команды на сервере **${interaction.guild.name}** были обновлены!` });
    }
    catch (err) {
      console.error(err);
      interaction.reply({ content: `При обновлении слэш команд на сервере **${interaction.guild.name}** произошла ошибка!` });
    }
  }
  ...
  if (server) {
    try {
      ...
      interaction.reply({ content: `Слэш команды на сервере **${await bot.guilds.resolve(server)}** были обновлены!` });
    }
    catch (err) {
      console.error(err);
      interaction.reply({ content: `При обновлении слэш команд на сервере **${await bot.guilds.resolve(server)}** произошла ошибка!` });
    }
  }
